...
<div class="bg-white rounded-lg shadow-lg p-6 max-w-4xl mx-auto">
    <h2 class="text-2xl font-bold text-gray-800 mb-6">Formulir Input Barang Masuk</h2>
    ...
    <form action="{{ route('barang-masuk.store') }}" method="POST">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                @error('tgl_masuk') <span class="text-red-500 text-sm mt-1">{{ $message }}</span> @enderror
            </div>
            ...
            <div>
                ...
                <input type="number" name="jumlah_masuk" id="jumlah_masuk" value="{{ old('jumlah_masuk') }}" min="1" required
                       class="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm">
                @error('jumlah_masuk') <span class="text-red-500 text-sm mt-1">{{ $message }}</span> @enderror
            </div>
            ...
            <div>
                ...
                <input type="number" name="stok_minimal" id="stok_minimal" value="{{ old('stok_minimal', 5) }}" min="0" required
                       class="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm">
            ...
            <div>
                ...
                <div class="mt-2 flex space-x-4">
                    <label class="inline-flex items-center"><input type="radio" name="kondisi" value="Baik" class="form-radio" {{ old('kondisi', 'Baik') == 'Baik' ? 'checked' : '' }}><span class="ml-2">Baik</span></label>
                    <label class="inline-flex items-center"><input type="radio" name="kondisi" value="Rusak" class="form-radio" {{ old('kondisi') == 'Rusak' ? 'checked' : '' }}><span class="ml-2">Rusak</span></label>
                </div>
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="info_maintenance" id="info_maintenance" rows="3"
                          class="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm">{{ old('info_maintenance') }}</textarea>
            </div>
        </div>
