...
<div class="bg-white rounded-lg shadow-lg p-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500">Daftar barang yang sedang dipinjam dan belum dikembalikan.</p>
        </div>
        @can('manage-transactions')
        <a href="{{ route('peminjaman.create') }}" class="no-print mt-3 sm:mt-0 inline-flex items-center px-4 py-2 bg-blue-600 border rounded-md font-semibold text-xs text-white uppercase hover:bg-blue-700">
            Input Peminjaman
        </a>
        @endcan
    </div>
    ...
    <div class="overflow-x-auto border border-gray-200 rounded-lg">
        <table id="dataTable" class="min-w-full bg-white divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    <th scope="col" class="py-3 px-4 text-left text-xs font-semibold text-gray-500 uppercase">Tgl Pinjam</th>
            ...
            <tbody id="tableBody" class="divide-y divide-gray-200">
                @forelse ($peminjamans as $item)
                    <tr>
                        <td class="py-4 px-4 whitespace-nowrap text-sm text-gray-500" data-tgl="{{ $item->tanggal_peminjaman }}">
                            <div>{{ \Carbon\Carbon::parse($item->tanggal_peminjaman)->isoFormat('DD MMM YYYY') }}</div>
                            <div class="text-xs text-gray-400 font-mono">{{ $item->kode_peminjaman }}</div>
                        </td>
                        <td class="py-4 px-4 whitespace-nowrap text-sm text-gray-800 font-medium">{{ $item->nama_peminjam }}</td>
                        <td class="py-4 px-4 whitespace-nowrap text-sm text-gray-500">{{ $item->barang->nama_barang }}</td>
                        <td class="py-4 px-4 whitespace-nowrap text-center text-sm text-gray-800">{{ $item->jumlah }} {{ $item->satuan->nama_satuan }}</td>
                        ...
                        <td class="no-print py-4 px-4 whitespace-nowrap text-sm">
                            <div class="flex items-center space-x-2">
                                <a href="{{ route('peminjaman.show', $item) }}" class="p-2 bg-gray-100 hover:bg-gray-200 rounded-full" title="Lihat Detail">
                                    <svg class="w-5 h-5 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                </a>
                                @can('manage-transactions')
                                <a href="{{ route('peminjaman.edit', $item) }}" class="p-2 bg-yellow-100 hover:bg-yellow-200 rounded-full" title="Edit">
                                ...
                                <form action="{{ route('peminjaman.destroy', $item) }}" method="POST" onsubmit="return confirm('Yakin batalkan peminjaman ini?');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="p-2 bg-red-100 hover:bg-red-200 rounded-full" title="Batalkan">
                ...
                @empty
                    <tr><td colspan="6" class="text-center py-10 text-gray-500">Tidak ada data peminjaman yang sedang aktif.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
