...
<div class="bg-white rounded-lg shadow-lg p-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
        <div>
            <h2 class="text-2xl font-bold text-gray-800">Data Peminjaman Barang</h2>
            <p class="mt-1 text-sm text-gray-500">Daftar semua barang yang sedang atau pernah dipinjam.</p>
        </div>
        @canany(['is-toolman', 'is-guru'])
            <a href="{{ route('peminjaman.create') }}" class="flex items-center bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded-lg shadow-md mt-4 sm:mt-0">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-center mb-4 space-y-2 sm:space-y-0">
        <div class="relative w-full sm:max-w-xs">
            ...
            <input type="text" id="searchInput" class="block w-full pl-10 pr-3 py-2 border border-gray-300 rounded-md leading-5 bg-white placeholder-gray-500 focus:outline-none focus:placeholder-gray-400 focus:ring-1 focus:ring-blue-500 focus:border-blue-500 sm:text-sm" placeholder="Cari peminjaman...">
        </div>
    ...
    <div class="overflow-x-auto border border-gray-200 rounded-lg">
        ...
        <table id="dataTable" class="min-w-full bg-white divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    <th scope="col" class="py-3 px-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider whitespace-nowrap">Kode Pinjam</th>
                    ...
                    <th scope="col" class="py-3 px-4 text-center text-xs font-semibold text-gray-500 uppercase tracking-wider whitespace-nowrap">Jumlah</th>
                    <th scope="col" class="py-3 px-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider whitespace-nowrap">Tgl Pinjam</th>
                    <th scope="col" class="py-3 px-4 text-center text-xs font-semibold text-gray-500 uppercase tracking-wider whitespace-nowrap">Status</th>
            ...
            <tbody id="tableBody" class="divide-y divide-gray-200">
                @forelse ($peminjamans as $item)
                    <tr>
                        <td class="py-4 px-4 text-sm text-gray-500 whitespace-nowrap">{{ $item->kode_peminjaman }}</td>
                        <td class="py-4 px-4 text-sm text-gray-800 whitespace-nowrap">{{ $item->nama_peminjam }}</td>
                        <td class="py-4 px-4 text-sm text-gray-500 whitespace-nowrap">{{ $item->barang->kode_barang ?? 'N/A' }}</td>

                        {{-- Barcode Barang --}}
                        <td class="py-4 px-4 text-center">
                            <div class="flex flex-col items-center">
                                {!! DNS1D::getBarcodeHTML($item->barang->kode_barang, 'C128') !!}
                                <div class="text-xs font-semibold mt-2">{{ $item->barang->kode_barang ?? 'N/A' }}</div>
                            </div>
                        </td>

                        <td class="py-4 px-4 text-sm text-gray-800 whitespace-nowrap">{{ $item->barang->nama_barang ?? 'N/A' }}</td>
                        <td class="py-4 px-4 text-sm text-gray-500 whitespace-nowrap">{{ $item->barang->kategoriBarang->nama_kategori ?? 'N/A' }}</td>
                        <td class="py-4 px-4 text-center text-sm text-gray-800 whitespace-nowrap">{{ $item->jumlah }} {{ $item->satuan->nama_satuan ?? '' }}</td>
                        
                        {{-- Tanggal Pinjam --}}
                        <td class="py-4 px-4 text-sm text-gray-500 whitespace-nowrap" data-tgl="{{ $item->tanggal_peminjaman->format('Y-m-d') }}">{{ $item->tanggal_peminjaman->format('d M Y') }}</td>
                        ...
                        <td class="py-4 px-4 whitespace-nowrap">
                            <div class="flex items-center space-x-2">
                                <a href="{{ route('peminjaman.show', $item->id) }}" class="p-2 bg-gray-100 hover:bg-gray-200 rounded-full" title="Lihat Detail">
                                    <svg class="w-5 h-5 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                </a>
                                @canany(['is-toolman', 'is-guru'])
                                    @if (!$item->pengembalian)
                                        <a href="{{ route('peminjaman.edit', $item->id) }}" class="p-2 bg-blue-100 hover:bg-blue-200 rounded-full" title="Edit">
                                        ...
                                        <form action="{{ route('peminjaman.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Yakin hapus data ini?');">@csrf @method('DELETE')<button type="submit" class="p-2 bg-red-100 hover:bg-red-200 rounded-full" title="Hapus">
                ...
                @empty
                    <tr><td colspan="10" class="text-center py-10 text-gray-500">Belum ada data peminjaman.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
