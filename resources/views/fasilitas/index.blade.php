@extends('layouts.app')
@section('title', 'Fasilitas di ' . $ruangan->nama_ruangan)
@section('content')
<div class="bg-white rounded-lg shadow-lg p-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
        <div>
            <h2 class="text-2xl font-bold text-gray-800">Fasilitas {{ $ruangan->nama_ruangan }} ({{ $ruangan->kode_ruangan }})</h2>
            <p class="mt-1 text-sm text-gray-500">
                Daftar semua fasilitas yang berada di ruangan ini.
                @if($ruangan->jurusan)
                    - Jurusan {{ $ruangan->jurusan->nama_jurusan }}
                @endif
            </p>
        </div>
        <div class="flex items-center space-x-2 mt-4 sm:mt-0">
            <a href="{{ route('ruangan.index') }}" class="flex items-center bg-gray-200 hover:bg-gray-300 text-gray-800 font-bold py-2 px-4 rounded-lg shadow-md">
                Kembali
            </a>
            @can('is-toolman')
                <a href="{{ route('fasilitas.create', $ruangan->id) }}" class="flex items-center bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded-lg shadow-md">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-center mb-4 space-y-2 sm:space-y-0">
        <div class="relative w-full sm:max-w-xs">
            ...
            <input type="text" id="searchInput" class="block w-full pl-10 pr-3 py-2 border border-gray-300 rounded-md leading-5 bg-white placeholder-gray-500 focus:outline-none focus:placeholder-gray-400 focus:ring-1 focus:ring-blue-500 focus:border-blue-500 sm:text-sm" placeholder="Cari fasilitas...">
        </div>
            ...
            <tbody id="tableBody" class="divide-y divide-gray-200">
                @forelse ($ruangan->barangs as $barang)
                    <tr>
                        ...
                        <td class="py-4 px-4 text-center text-sm text-blue-600 font-medium whitespace-nowrap">{{ $barang->pivot->jumlah }} {{ $barang->satuan->nama_satuan ?? '' }}</td>
                        <td class="py-4 px-4 whitespace-nowrap">
                            <div class="flex items-center space-x-2">
                                @can('is-toolman')
                                    <a href="{{ route('fasilitas.edit', [$ruangan->id, $barang->kode_barang]) }}" class="p-2 bg-blue-100 hover:bg-blue-200 rounded-full" title="Edit Jumlah"><svg class="w-5 h-5 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg></a>
                                    <form action="{{ route('fasilitas.destroy', [$ruangan->id, $barang->kode_barang]) }}" method="POST" onsubmit="return confirm('Yakin hapus fasilitas ini dari ruangan?');">@csrf @method('DELETE')<button type="submit" class="p-2 bg-red-100 hover:bg-red-200 rounded-full" title="Hapus"><svg class="w-5 h-5 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg></button></form>
                                @endcan
                            </div>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="7" class="text-center py-10 text-gray-500">Belum ada fasilitas di ruangan ini.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
