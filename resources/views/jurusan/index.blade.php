...
<div class="bg-white rounded-lg shadow-lg p-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500">Daftar semua jurusan yang tersedia di sekolah.</p>
        </div>
        @can('is-toolman')
            <a href="{{ route('jurusan.create') }}" class="flex items-center bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded-lg shadow-md mt-4 sm:mt-0">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-center mb-4 space-y-2 sm:space-y-0">
        <div class="relative w-full sm:max-w-xs">
            ...
            <input type="text" id="searchInput" class="block w-full pl-10 pr-3 py-2 border border-gray-300 rounded-md leading-5 bg-white placeholder-gray-500 focus:outline-none focus:placeholder-gray-400 focus:ring-1 focus:ring-blue-500 focus:border-blue-500 sm:text-sm" placeholder="Cari jurusan...">
        </div>
        ...
        <table id="dataTable" class="min-w-full bg-white divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th scope="col" class="py-3 px-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider whitespace-nowrap">Nama Jurusan</th>
                    <th scope="col" class="py-3 px-4 text-center text-xs font-semibold text-gray-500 uppercase tracking-wider whitespace-nowrap">Jumlah Ruangan</th>
            ...
            <tbody id="tableBody" class="divide-y divide-gray-200">
                @forelse ($jurusans as $item)
                    <tr>
                        ...
                        <td class="py-4 px-4 text-sm text-gray-800 whitespace-nowrap">{{ $item->nama_jurusan }}</td>
                        <td class="py-4 px-4 text-center text-sm text-gray-800 whitespace-nowrap">{{ $item->ruangans_count }}</td>
                        <td class="py-4 px-4 whitespace-nowrap">
                            <div class="flex items-center space-x-2">
                                @can('is-toolman')
                                    <a href="{{ route('jurusan.edit', $item->id) }}" class="p-2 bg-blue-100 hover:bg-blue-200 rounded-full" title="Edit">
                ...
                @empty
                    <tr><td colspan="4" class="text-center py-10 text-gray-500">Belum ada data jurusan.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
