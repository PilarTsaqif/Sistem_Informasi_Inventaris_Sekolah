...
<div class="bg-white rounded-lg shadow-lg p-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
        <div>
            <h2 class="text-2xl font-bold text-gray-800">Manajemen Pemasok</h2>
            <p class="mt-1 text-sm text-gray-500">Daftar semua pemasok barang.</p>
        </div>
        @can('manage-masters')
        <a href="{{ route('master.pemasok.create') }}" class="no-print mt-3 sm:mt-0 inline-flex items-center px-4 py-2 bg-blue-600 border rounded-md font-semibold text-xs text-white uppercase hover:bg-blue-700">
            Tambah Pemasok
        </a>
        @endcan
    </div>
    ...
    <div class="no-print flex flex-col sm:flex-row justify-between items-center mb-4 space-y-2 sm:space-y-0">
        <div class="relative w-full sm:max-w-xs">
            ...
            <input type="text" id="searchInput" class="block w-full pl-10 pr-3 py-2 border border-gray-300 rounded-md bg-white placeholder-gray-500 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" placeholder="Cari pemasok...">
        </div>
    ...
    <div class="overflow-x-auto border border-gray-200 rounded-lg">
        <table id="dataTable" class="min-w-full bg-white divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    <th scope="col" class="py-3 px-4 text-left text-xs font-semibold text-gray-500 uppercase">Nama Pemasok</th>
            ...
            <tbody id="tableBody" class="divide-y divide-gray-200">
                @forelse ($pemasoks as $pemasok)
                    <tr>
                        <td class="py-4 px-4 whitespace-nowrap text-sm text-gray-800 font-medium">{{ $pemasok->nama_pemasok }}</td>
                        <td class="no-print py-4 px-4 whitespace-nowrap text-sm">
                            <div class="flex items-center space-x-2">
                                @can('manage-masters')
                                <a href="{{ route('master.pemasok.edit', $pemasok) }}" class="p-2 bg-yellow-100 hover:bg-yellow-200 rounded-full" title="Edit">
                                ...
                                <form action="{{ route('master.pemasok.destroy', $pemasok) }}" method="POST" onsubmit="return confirm('Yakin hapus pemasok ini?');">
                                    @csrf
                                    @method('DELETE')
                ...
                @empty
                    <tr><td colspan="2" class="text-center py-10 text-gray-500">Tidak ada data pemasok.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
