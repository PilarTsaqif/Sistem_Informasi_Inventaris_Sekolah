...
<div class="bg-white rounded-lg shadow-lg p-6 max-w-2xl mx-auto">
    <h2 class="text-2xl font-bold text-gray-800 mb-6">Formulir Tambah Ruangan Baru</h2>
    ...
    <form action="{{ route('master.ruangan.store') }}" method="POST">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            {{-- Kode Ruangan --}}
            <div>
                <label for="kode_ruangan" class="block text-sm font-medium text-gray-700">Kode Ruangan <span class="text-red-600">*</span></label>
                ...
                @error('kode_ruangan') <span class="text-red-500 text-sm mt-1">{{ $message }}</span> @enderror
            </div>
            ...
            <div>
                ...
                <select name="id_jurusan" id="id_jurusan"
                    ...
                    @foreach($jurusans as $jurusan)
                        <option value="{{ $jurusan->id }}" {{ old('id_jurusan') == $jurusan->id ? 'selected' : '' }}>{{ $jurusan->nama_jurusan }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <input type="text" name="kode_rps" id="kode_rps" value="{{ old('kode_rps') }}"
                       class="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm">
            </div>
        </div>
