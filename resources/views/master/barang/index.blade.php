...
<div class="bg-white rounded-lg shadow-lg p-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500">Daftar semua barang yang terdaftar di dalam sistem.</p>
        </div>
        @can('manage-masters')
        <a href="{{ route('master.barang.create') }}" class="no-print mt-3 sm:mt-0 inline-flex items-center px-4 py-2 bg-blue-600 border rounded-md font-semibold text-xs text-white uppercase hover:bg-blue-700">
        ...
        <table id="dataTable" class="min-w-full bg-white divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="py-3 px-4 text-left text-xs font-semibold text-gray-500 uppercase">Kategori</th>
                    <th class="py-3 px-4 text-left text-xs font-semibold text-gray-500 uppercase">Satuan</th>
                    <th class="py-3 px-4 text-center text-xs font-semibold text-gray-500 uppercase">Barcode</th> {{-- Kolom Baru --}}
                    <th class="no-print py-3 px-4 text-left text-xs font-semibold text-gray-500 uppercase w-40">Aksi</th>
                </tr>
            </thead>
            <tbody id="tableBody" class="divide-y divide-gray-200">
                @forelse ($barangs as $barang)
                    <tr>
                        ...
                        <td class="py-4 px-4 whitespace-nowrap text-sm text-gray-500">{{ $barang->satuan->nama_satuan ?? 'N/A' }}</td>
                        {{-- [BARU] Kolom untuk menampilkan barcode --}}
                        ...
                        <td class="no-print py-4 px-4 whitespace-nowrap text-sm">
                            <div class="flex items-center space-x-2">
                                <a href="{{ route('master.barang.show', $barang) }}" class="p-2 bg-gray-100 hover:bg-gray-200 rounded-full" title="Lihat">
                                    <svg class="w-5 h-5 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                </a>
                                @can('manage-masters')
                                <a href="{{ route('master.barang.edit', $barang) }}" class="p-2 bg-yellow-100 hover:bg-yellow-200 rounded-full" title="Edit">
                                ...
                                <form action="{{ route('master.barang.destroy', $barang) }}" method="POST" onsubmit="return confirm('Yakin ingin hapus?');">
                                    @csrf
                                    @method('DELETE')
                ...
                @empty
                    <tr><td colspan="5" class="text-center py-10 text-gray-500">Tidak ada data untuk ditampilkan.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
