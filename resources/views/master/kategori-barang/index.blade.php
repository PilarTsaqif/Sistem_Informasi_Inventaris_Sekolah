...
<div class="bg-white rounded-lg shadow-lg p-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500">Daftar semua kategori untuk pengelompokan barang.</p>
        </div>
        @can('manage-masters')
        <a href="{{ route('master.kategori-barang.create') }}" class="no-print mt-3 sm:mt-0 inline-flex items-center px-4 py-2 bg-blue-600 border rounded-md font-semibold text-xs text-white uppercase hover:bg-blue-700">
            Tambah Kategori
        </a>
        @endcan
    </div>
    ...
    <div class="no-print flex flex-col sm:flex-row justify-between items-center mb-4 space-y-2 sm:space-y-0">
        <div class="relative w-full sm:max-w-xs">
            ...
            <input type="text" id="searchInput" class="block w-full pl-10 pr-3 py-2 border border-gray-300 rounded-md bg-white placeholder-gray-500 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" placeholder="Cari kategori...">
        </div>
    ...
    <div class="overflow-x-auto border border-gray-200 rounded-lg">
        <table id="dataTable" class="min-w-full bg-white divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    <th scope="col" class="py-3 px-4 text-left text-xs font-semibold text-gray-500 uppercase">Nama Kategori</th>
            ...
            <tbody id="tableBody" class="divide-y divide-gray-200">
                @forelse ($kategoriBarangs as $kategori)
                    <tr>
                        <td class="py-4 px-4 whitespace-nowrap text-sm text-gray-800 font-medium">{{ $kategori->nama_kategori }}</td>
                        <td class="no-print py-4 px-4 whitespace-nowrap text-sm">
                            <div class="flex items-center space-x-2">
                                <a href="{{ route('master.kategori-barang.show', $kategori) }}" class="p-2 bg-gray-100 hover:bg-gray-200 rounded-full" title="Lihat">
                                    <svg class="w-5 h-5 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                </a>
                                @can('manage-masters')
                                <a href="{{ route('master.kategori-barang.edit', $kategori) }}" class="p-2 bg-yellow-100 hover:bg-yellow-200 rounded-full" title="Edit">
                                ...
                                <form action="{{ route('master.kategori-barang.destroy', $kategori) }}" method="POST" onsubmit="return confirm('Yakin hapus kategori ini?');">
                                    @csrf
                                    @method('DELETE')
                ...
                @empty
                    <tr><td colspan="2" class="text-center py-10 text-gray-500">Tidak ada data kategori.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
